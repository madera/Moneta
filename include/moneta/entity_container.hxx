...
namespace moneta {
	...
	template <class EntityType, class ComparatorType = std::less<EntityType> >
	class entity_container : boost::noncopyable {

		template <class EntityType>
		struct entry {
			entry()
			 : state(0) {
				entity = make_entity<EntityType>();
			}

			entry(const EntityType& entity_)
			 : state(0), entity(entity_) {};
