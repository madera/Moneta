...
BOOST_AUTO_TEST_CASE(shell_encoder_test_1) {	
	...
	int result = moneta::codec::shell_encoder()(boost::begin(buffer), boost::end(buffer), make_composite());
...
BOOST_AUTO_TEST_CASE(shell_encoder_test_2) {
	...
	int result = moneta::codec::shell_encoder()(boost::begin(buffer), boost::end(buffer), moneta::make_entity<A>());
	BOOST_CHECK_EQUAL(result, expected.size());
	BOOST_CHECK_EQUAL(buffer, expected);
}
